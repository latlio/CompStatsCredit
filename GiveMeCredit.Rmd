...
cs_training <- read_csv("~/Desktop/CompStats/cs-training.csv")
...
cs_training_clean <- cs_training_clean %>%
  mutate(Distributedincome = MonthlyIncome/(1+NumberOfDependents),
         ...
         RUULIncome = log(RevolvingUtilizationOfUnsecuredLines/(MonthlyIncome+1)+1), #Housing Insolvency paper
...
par(mfrow = c(1,2))
ggplot(cs_training, aes(LogRUUL, fill = as.factor(SeriousDlqin2yrs))) + geom_density(alpha = 0.2)+ xlim(0,1)
ggplot(cs_training, aes(age, fill = as.factor(SeriousDlqin2yrs))) + geom_density(alpha = 0.2)
ggplot(cs_training_clean, aes(RevolvingUtilizationOfUnsecuredLines, fill = as.factor(SeriousDlqin2yrs))) + geom_density(alpha = 0.2)
ggplot(cs_training_clean, aes(RUULIncome, fill = as.factor(SeriousDlqin2yrs))) + geom_density(alpha = 0.2)
```
